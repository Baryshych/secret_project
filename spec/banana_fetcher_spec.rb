...
describe BananaFetcher do
  ...
  context 'caching' do
    ...
    it 'provides cached bananas with lookback' do
      fetcher = BananaFetcher.new
      bananas = fetcher.fetch_with_lookback_cache(0)
      fetcher.fetch_with_lookback_cache(1)
      fetcher.collection = fetcher.seed[11...30]
      # and the biggest flaw - inconsistency is seen here
      new_bananas = fetcher.fetch_with_lookback_cache(0)
      expect(bananas).to eq new_bananas
    end
  end
end